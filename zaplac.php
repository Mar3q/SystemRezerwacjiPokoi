...
<?php
    ...
    if ( !(empty($_POST['id']))) {
		...
		$sql = "UPDATE RezerwacjePokoi set oplacono = 'tak' WHERE rezerwacjaid = ?"; //usuwam z wolnych pokoi
		$q = $pdo->prepare($sql);
        $q->execute(array($id));
...
<body>
		<div id="naglowekRezerwacji">Potwierdź płatność</div>
			<form id="formularzRezerwacji"  action="zaplac.php" method="post">
				<br>
